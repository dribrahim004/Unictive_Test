...
namespace Unictive.Controllers
{
    ...
    [Route("[controller]")]
    public class UpdateMember : ControllerBase
    {
        private readonly ILogger<UpdateMember> _logger;

        public UpdateMember(ILogger<UpdateMember> logger)
        {
            _logger = logger;
        }

        [HttpGet(Name = "UpdateMember")]
        public Response Get(Member member)
        {
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(connString))
                {
                    ...
                    String sql = "exec USP_MEMBER_UPADTE '" + member.Id + "','" + member.Nama + "','" + member.Email + "','" + member.Phone + "'";
